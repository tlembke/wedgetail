module InPlaceMacrosHelper
  ...
  def in_place_delete_editor(field_id, options = {})
    function =  "new Ajax.InPlaceDeleteEditor("
    function << "'#{field_id}', "
    function << "'#{url_for(options[:url])}'"
  ...
  def in_place_delete_editor_field(object, method, tag_options = {}, in_place_editor_options = {})
    instance_tag = ::ActionView::Helpers::InstanceTag.new(object, method, self)
    tag_options = {:tag => "span",
                   :id => "#{object}_#{method}_#{instance_tag.object.id}_in_place_editor",
                   :class => "in_place_editor_field"}.merge!(tag_options)
    ...
    return tag + in_place_delete_editor(tag_options[:id], in_place_editor_options)
  end
end
